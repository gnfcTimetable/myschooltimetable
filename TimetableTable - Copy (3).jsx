...
const TimetableTable = () => {
  ...
  const [currentCommonRoutine, setCurrentCommonRoutine] = useState("");
  ...
  const updateSlot = () => {
    ...
    const activeSlot = findCurrentSlot(todaySchedule);

if (activeSlot) {
  setCurrentSlot(activeSlot);
  setCurrentSlotTime(activeSlot.time);
  setCurrentCommonRoutine(activeSlot.commonRoutine || "No common routine at this time");
} else {
  setCurrentSlot(null);
  setCurrentSlotTime("No active slot");
  setCurrentCommonRoutine("No common routine at this time");
}

  };

  updateSlot();  // Call updateSlot immediately when useEffect runs

  const interval = setInterval(updateSlot, 60000);
  return () => clearInterval(interval);
}, [timetableData]);  // Depend on timetableData to update correctly
  useEffect(() => {
    const updateTime = () => {
      const now = new Date();
...
<h4 className="common-routine" align="center">📝 {currentCommonRoutine}</h4>
